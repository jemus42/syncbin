...
local({
  ...
  if (grepl("^Rocky Linux", os_version)) {
     p3m_url <- sprintf("https://p3m.dev/cran/latest/bin/linux/rhel9-%s/%s", R.version["arch"], substr(getRversion(), 1, 3))
  }
...
# User agent for P3M compatibility on platforms where it's unset
